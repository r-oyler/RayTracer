...
public class Teapot {
	...
	static int kTeapotNumPatches = 32; 
	static int kTeapotNumVertices = 306; 

	static int[][] teapotPatches = { 
	...
	static void generateTeapot(ArrayList<MatObject> objects) {
		
		int divs = 16/4;
		
		Vector[] p = new Vector[(divs+1)*(divs+1)];
		int[] nVertices = new int[divs*divs];
		int[] vertices = new int[divs*divs*4];
		Vector[] controlPoints = new Vector[16];
		
		for (int np = 0; np < kTeapotNumPatches; np++ ) {
			
			for (int i = 0; i < 16; i++) {
				double x = teapotVertices[teapotPatches[np][i]-1][0];
				double y = teapotVertices[teapotPatches[np][i]-1][1];
				double z = teapotVertices[teapotPatches[np][i]-1][2];
			...
			for (int j = 0, k = 0; j <= divs; j++) {
				...
				for (int i = 0; i <= divs; i++, k++) {
					p[k] = Bezier.evalBezierSurface(controlPoints, i/(double)divs, j/(double)divs);
				}
			...
			for (int i = 0; i < normals.length; i++) normals[i] = new Vector(1,0,0);
			
			PolyMesh pm = new PolyMesh(divs * divs, nVertices.clone(), vertices.clone(), p.clone(), normals.clone(), Material.BLUE);
