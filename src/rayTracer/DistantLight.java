...
public class DistantLight extends Light {
	...
	DistantLight(Vector c, Vector d) {
		color = c;
	...
	public Vector DiffuseSpecular(Ray ray, IntersectInfo info, Scene scene, Settings settings) {
		...
		Vector objectToLight = this.direction.timesConst(-1);
		...
		for (int i=0; visible  && i<scene.getObjects().size(); i++) {
			...
			if (!obj.material.isRefractive){	// Refractive materials don't cast shadows				
				...
				visible &= (shadowRayInfo.getTime()==Double.POSITIVE_INFINITY);
			}
		}
	...
	@Override
	public Light clone() {
		return new DistantLight(this.color.clone(),this.direction.clone());
	}
